...
if [[ "$PLATFORM" == windows || "$PLATFORM" == linux || "$PLATFORM" == all ]]; then
  [[ -f "$ASI_PLUGIN" ]] || { echo "Build the ASI plugin first: $ASI_PLUGIN" >&2; exit 1; }
  [[ -f "$HUD_ASI_PLUGIN" ]] || { echo "Build the centered-HUD ASI first: $HUD_ASI_PLUGIN" >&2; exit 1; }
...
make_tree() {
  ...
  if [[ "$platform" == windows ]]; then
    ...
    # Proton uses the same proven layout as Windows: Ultimate ASI Loader plus
    # separate core and FPS plugins. The FPS plugin itself is fetched and
    # adapted locally by the Linux installer because upstream does not grant
  ...
  if [[ "$platform" == windows ]]; then
    ...
    install -m0755 "$REPO_DIR/MGS4Ultra120-Linux-Setup.sh" \
      "$root/MGS4Ultra120-Linux-Setup.sh"
    install -m0755 "$REPO_DIR/MGS4Ultra120-Linux-Configure.sh" \
      "$root/MGS4Ultra120-Linux-Configure.sh"
    install -m0755 "$REPO_DIR/MGS4Ultra120-Linux-Uninstall.sh" \
      "$root/MGS4Ultra120-Linux-Uninstall.sh"
  fi
